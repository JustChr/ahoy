...
- [ ] Slice 4c — migrate setup/config sections into the shell (the 68 KB `setup.html`; biggest/riskiest — settings contracts + secret redaction). **DEFERRED** — keep legacy `/setup` as safety net until SPA parity verified on-device.
- [ ] Slice 4e — web-serial + update/OTA + first-run wizard. **DEFERRED** (rebuilding OTA upload / serial blind = brick/regression risk; legacy `/serial` `/update` stay reachable). History charts intentionally skipped: shipped `esp8266` env has **no `-DENABLE_HISTORY`**, so `/api/powerHistory` does not exist on the running firmware.
- [ ] **Full §11.5 parity checklist ticked**; settings migration verified (§13.3); MQTT/HA discovery untouched.
- [ ] **Retire standalone pages** (final step, only after the above verified at parity).
