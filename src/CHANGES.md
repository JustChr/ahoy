...
* OTA: quiesce the NRF radio + inverter communication during a flash too. They allocate transiently while parsing RF payloads, competing with Update.write for the ~13 KB heap; standing them down for the (~30 s) flash removes the last uncontrolled heap consumer during OTA. A dropped RF frame while flashing is harmless and they resume the instant the OTA flag clears. (MQTT publishing was already gated; this closes the remaining path.)
